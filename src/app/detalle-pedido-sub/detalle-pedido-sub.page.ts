...
@Component({
  selector: 'app-detalle-pedido-sub',
  templateUrl: './detalle-pedido-sub.page.html',
  styleUrls: ['./detalle-pedido-sub.page.scss'],
})
export class DetallePedidoSubPage implements OnInit {
  pedido;
  objetoDatosPedido;
  objListaProductos: any[]=[];
  pedidosSubPendientes:any []=[]; 
  tipoEstado: any;
  constructor(
    private http: HttpClient, private activatedRoute: ActivatedRoute,private pedidosService:PedidosService,private permisos:PermisosService
  ) {}

  async ngOnInit() {
    this.pedido = this.activatedRoute.snapshot.paramMap.get('idDetallePedido');
   ...
   // this.tipoEstado = await this.pedidosService.getStatus(idUsuario,1);
    //console.log(this.tipoEstado);
